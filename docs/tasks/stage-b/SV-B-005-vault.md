...
- `contact-credential-bytes`: [SV-B-002](SV-B-002-contact.md) остаётся owner contact wire; здесь владелец `capability.credentials.private-input` проверяет, что native password draft, CLI и vault сохраняют **исходные bytes** без Unicode normalization, перекодирования по шрифту или молчаливой замены. Общий UTF-8→Latin-1 decoder `src/client/sv/input/text-field.c` не является доказательством server wire mapping: сравнить baseline и реальные non-ASCII auth bytes вместе с [SV-B-006](SV-B-006-login.md). При неподтверждённом mapping оставлять draft и явную encoding error; не записывать преобразованный secret в vault.
