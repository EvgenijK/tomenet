...
#define SV_CONTACT_OUTPUT 256
#define SV_CONTACT_MOTD (120 * 23)
...
struct SvContact {
    ...
    unsigned rejection;
    unsigned char input[SV_CONTACT_INPUT_CAPACITY], output[SV_CONTACT_OUTPUT];
...
static int skip_string(const unsigned char *bytes, size_t length, size_t *cursor)
{
    if (*cursor >= length) return 0;
    size_t available = length - *cursor;
    if (available > MAX_CHARS) available = MAX_CHARS;
    const unsigned char *end = memchr(bytes + *cursor, 0, available);
    if (!end) return available == MAX_CHARS ? -1 : 0;
    *cursor = (size_t)(end - bytes) + 1;
    return 1;
}
static int parse_setup(const SvContact *contact, size_t *used)
{
    ...
    for (;;) {
        if (contact->state == SV_CONTACT_WAIT_CONTACT) {
            ...
            if (flags & 2) for (int i = 0; i < 6; ++i)
                contact->version[i] = (int)get32(contact->input + 10 + 4 * i);
            else {
